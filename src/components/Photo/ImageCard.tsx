...
interface ImageCardProps {
  ...
  src: string;
  alt: string;
  isSelected: boolean; // 新增的选中状态
  date: string;
  width?:number;
  height?:number;
  className?: string;
  style?:React.CSSProperties;
  onSelect?: (id: string, isSelected:boolean, date:string) => void; // 新增的选中事件处理函数
}

const ImageCard: React.FC<ImageCardProps> = ({ width, height, id, src,date, alt, className = "", onSelect, isSelected}) => {
  const navigate = useNavigate();

  // 切换选中状态
  const toggleSelection = () => {
    onSelect && onSelect(id, !isSelected, date); // 触发回调
  };
    ...
    >
      ...
      {isSelected ? (
        <img
          src={src}
          alt={alt}
          className={className + "w-auto h-64 object-cover transition-transform duration-500 scale-90"}
        ...
        <img
          ...
          style={{width:width, height:height}}
          onClick={()=>navigate(`/homephotodetail/${id}`)} // 点击图片时触发 toggleSelection 函数
        />
      )}
