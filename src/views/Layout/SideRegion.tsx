...
const selectItems = [
  ...
  { key: "photogallery", icon: <UploadOutlined />, label: <NavLink to="/photogallery">照片画廊</NavLink> },
  { key: "LLM", icon: <SettingOutlined />, label: <NavLink to="/LLM">大模型</NavLink> }, // 微前端实现
];
...
const SideRegion: React.FC<ThemeWrapperProps> = ({ token, isDarkMode, collapsed, setCollapsed, toggleButtonRef }) => {
  ...
  return (
    <div
      ref={sideRef}
      className={`fixed top-[64px] left-0 z-40 h-[calc(100vh-64px)] w-[256px] transform transition-transform duration-300 ease-in-out border-r-1 border-gray-200
        bg-white
        ${!collapsed ? "translate-x-0" : "-translate-x-full"}`}
        style={{background:token.colorBgBase,borderColor: token.colorBorder}}
    >
